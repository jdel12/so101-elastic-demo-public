...
docker build -f infra/docker/Dockerfile.inference -t so101-inference:latest .
docker tag so101-inference:latest ${REGISTRY}/so101-inference:latest
docker push ${REGISTRY}/so101-inference:latest

echo "=== Building MCP server image ==="
docker build -f infra/docker/Dockerfile.mcp -t so101-mcp-server:latest .
...
docker build -f infra/docker/Dockerfile.motor -t so101-motor:latest .
...
echo ""
echo "To deploy:"
echo "  kubectl apply -f infra/k8s/robot-brain.yaml"
